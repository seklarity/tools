...
{
	sekcee_stdout(L"%*s" L"**BLOB**\n", level << 1, L"");
	if(blob)
	{
		sekcee_stdout(L"%*s" L"  dwVersion          : %08x - %u\n", level << 1, L"", blob->dwVersion, blob->dwVersion);
		sekcee_stdout(L"%*s" L"  guidProvider       : ", level << 1, L""); kull_m_string_displayGUID(&blob->guidProvider); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  dwMasterKeyVersion : %08x - %u\n", level << 1, L"", blob->dwMasterKeyVersion, blob->dwMasterKeyVersion);
		sekcee_stdout(L"%*s" L"  guidMasterKey      : ", level << 1, L""); kull_m_string_displayGUID(&blob->guidMasterKey); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  dwFlags            : %08x - %u (", level << 1, L"", blob->dwFlags, blob->dwFlags);
		kull_m_dpapi_displayBlobFlags(blob->dwFlags);
		sekcee_stdout(L")\n");
		sekcee_stdout(L"%*s" L"  dwDescriptionLen   : %08x - %u\n", level << 1, L"", blob->dwDescriptionLen, blob->dwDescriptionLen);
		sekcee_stdout(L"%*s" L"  szDescription      : %s\n", level << 1, L"", blob->szDescription);
		sekcee_stdout(L"%*s" L"  algCrypt           : %08x - %u (%s)\n", level << 1, L"", blob->algCrypt, blob->algCrypt, kull_m_crypto_algid_to_name(blob->algCrypt));
		sekcee_stdout(L"%*s" L"  dwAlgCryptLen      : %08x - %u\n", level << 1, L"", blob->dwAlgCryptLen, blob->dwAlgCryptLen);
		sekcee_stdout(L"%*s" L"  dwSaltLen          : %08x - %u\n", level << 1, L"", blob->dwSaltLen, blob->dwSaltLen);
		sekcee_stdout(L"%*s" L"  pbSalt             : ", level << 1, L""); kull_m_string_wprintf_hex(blob->pbSalt, blob->dwSaltLen, 0); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  dwHmacKeyLen       : %08x - %u\n", level << 1, L"", blob->dwHmacKeyLen, blob->dwHmacKeyLen);
		sekcee_stdout(L"%*s" L"  pbHmackKey         : ", level << 1, L""); kull_m_string_wprintf_hex(blob->pbHmackKey, blob->dwHmacKeyLen, 0); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  algHash            : %08x - %u (%s)\n", level << 1, L"", blob->algHash, blob->algHash, kull_m_crypto_algid_to_name(blob->algHash));
		sekcee_stdout(L"%*s" L"  dwAlgHashLen       : %08x - %u\n", level << 1, L"", blob->dwAlgHashLen, blob->dwAlgHashLen);
		sekcee_stdout(L"%*s" L"  dwHmac2KeyLen      : %08x - %u\n", level << 1, L"", blob->dwHmac2KeyLen, blob->dwHmac2KeyLen);
		sekcee_stdout(L"%*s" L"  pbHmack2Key        : ", level << 1, L""); kull_m_string_wprintf_hex(blob->pbHmack2Key, blob->dwHmac2KeyLen, 0); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  dwDataLen          : %08x - %u\n", level << 1, L"", blob->dwDataLen, blob->dwDataLen);
		sekcee_stdout(L"%*s" L"  pbData             : ", level << 1, L""); kull_m_string_wprintf_hex(blob->pbData, blob->dwDataLen, 0); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  dwSignLen          : %08x - %u\n", level << 1, L"", blob->dwSignLen, blob->dwSignLen);
		sekcee_stdout(L"%*s" L"  pbSign             : ", level << 1, L""); kull_m_string_wprintf_hex(blob->pbSign, blob->dwSignLen, 0); sekcee_stdout(L"\n\n");
	}
}
...
{
	sekcee_stdout(L"%*s" L"**MASTERKEY**\n", level << 1, L"");
	if(masterkey)
	{
		sekcee_stdout(L"%*s" L"  dwVersion        : %08x - %u\n", level << 1, L"", masterkey->dwVersion, masterkey->dwVersion);
		sekcee_stdout(L"%*s" L"  salt             : ", level << 1, L""); kull_m_string_wprintf_hex(masterkey->salt, sizeof(masterkey->salt), 0); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  rounds           : %08x - %u\n", level << 1, L"", masterkey->rounds, masterkey->rounds);
		...
		sekcee_stdout(L"%*s" L"  pbKey            : ", level << 1, L""); kull_m_string_wprintf_hex(masterkey->pbKey, masterkey->__dwKeyLen, 0); sekcee_stdout(L"\n\n");
	}
}
...
{
	sekcee_stdout(L"%*s" L"**CREDHIST INFO**\n", level << 1, L"");
	if(credhist)
	{
		sekcee_stdout(L"%*s" L"  dwVersion        : %08x - %u\n", level << 1, L"", credhist->dwVersion, credhist->dwVersion);
		sekcee_stdout(L"%*s" L"  guid             : ", level << 1, L""); kull_m_string_displayGUID(&credhist->guid); sekcee_stdout(L"\n\n");
	}
}
...
{
	sekcee_stdout(L"%*s" L"**DOMAINKEY**\n", level << 1, L"");
	if(domainkey)
	{
		sekcee_stdout(L"%*s" L"  dwVersion        : %08x - %u\n", level << 1, L"", domainkey->dwVersion, domainkey->dwVersion);
		sekcee_stdout(L"%*s" L"  dwSecretLen      : %08x - %u\n", level << 1, L"", domainkey->dwSecretLen, domainkey->dwSecretLen);
		sekcee_stdout(L"%*s" L"  dwAccesscheckLen : %08x - %u\n", level << 1, L"", domainkey->dwAccesscheckLen, domainkey->dwAccesscheckLen);
		sekcee_stdout(L"%*s" L"  guidMasterKey    : ", level << 1, L""); kull_m_string_displayGUID(&domainkey->guidMasterKey); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  pbSecret         : ", level << 1, L""); kull_m_string_wprintf_hex(domainkey->pbSecret, domainkey->dwSecretLen, 0); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  pbAccesscheck    : ", level << 1, L""); kull_m_string_wprintf_hex(domainkey->pbAccesscheck, domainkey->dwAccesscheckLen, 0); sekcee_stdout(L"\n\n");
	}
}
...
void kull_m_dpapi_masterkeys_descr(DWORD level, PKULL_M_DPAPI_MASTERKEYS masterkeys)
{
	...
	if(masterkeys)
	{
		...
		if(masterkeys->BackupKey)
		{
			sekcee_stdout(L"%*s" L"[backupkey]\n", level << 1, L"");
			kull_m_dpapi_masterkey_descr(level + 1, masterkeys->BackupKey);
		}
		if(masterkeys->CredHist)
		{
			sekcee_stdout(L"%*s" L"[credhist]\n", level << 1, L"");
...
void kull_m_dpapi_credhist_descr(DWORD level, PKULL_M_DPAPI_CREDHIST credhist)
{
	...
	if(credhist)
	{
		...
		sekcee_stdout(L"%*s" L"  guid      : ", level << 1, L""); kull_m_string_displayGUID(&credhist->current.guid); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  dwNextLen : %08x - %u\n", level << 1, L"", credhist->current.dwNextLen, credhist->current.dwNextLen);
		for(i = 0; i < credhist->__dwCount; i++)
...
{
	sekcee_stdout(L"%*s" L"**CREDHIST ENTRY**\n", level << 1, L"");
	if(entry)
	{
		sekcee_stdout(L"%*s" L"  dwVersion : %08x - %u\n", level << 1, L"", entry->header.dwVersion, entry->header.dwVersion);
		sekcee_stdout(L"%*s" L"  guid      : ", level << 1, L""); kull_m_string_displayGUID(&entry->header.guid); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  dwNextLen : %08x - %u\n", level << 1, L"", entry->header.dwNextLen, entry->header.dwNextLen);
		
		sekcee_stdout(L"%*s" L"  dwType    : %08x - %u\n", level << 1, L"", entry->dwType, entry->dwType);
		sekcee_stdout(L"%*s" L"  algHash   : %08x - %u (%s)\n", level << 1, L"", entry->algHash, entry->algHash, kull_m_crypto_algid_to_name(entry->algHash));
		sekcee_stdout(L"%*s" L"  rounds    : %08x - %u\n", level << 1, L"", entry->rounds, entry->rounds);
		sekcee_stdout(L"%*s" L"  sidLen    : %08x - %u\n", level << 1, L"", entry->sidLen, entry->sidLen);
		sekcee_stdout(L"%*s" L"  algCrypt  : %08x - %u (%s)\n", level << 1, L"", entry->algCrypt, entry->algCrypt, kull_m_crypto_algid_to_name(entry->algCrypt));
		sekcee_stdout(L"%*s" L"  sha1Len   : %08x - %u\n", level << 1, L"", entry->sha1Len, entry->sha1Len);
		sekcee_stdout(L"%*s" L"  md4Len    : %08x - %u\n", level << 1, L"", entry->md4Len, entry->md4Len);
		
		sekcee_stdout(L"%*s" L"  Salt      : ", level << 1, L""); kull_m_string_wprintf_hex(entry->salt, sizeof(entry->salt), 0); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  Sid       : ", level << 1, L""); kull_m_string_displaySID(entry->pSid); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  pSecret   : ", level << 1, L""); kull_m_string_wprintf_hex(entry->pSecret, entry->__dwSecretLen, 0); sekcee_stdout(L"\n\n");
	}
}
						...
						if(CryptSetKeyParam(hSessionKey, KP_IV, rsa_buffer->buffer + rsa_buffer->cbMasterKey + 192 / 8, 0))
						{
							...
							if(des_buffer = (PKULL_M_DPAPI_DOMAIN_ACCESS_CHECK) LocalAlloc(LPTR, cbOutput))
							{
								...
								if(CryptDecrypt(hSessionKey, 0, FALSE, 0, (PBYTE) des_buffer, &cbOutput))
								{
									...
									////kull_m_string_wprintf_hex(des_buffer, outSize, 1 | (16 << 16)); sekcee_stdout(L"\n");
									//sekcee_stdout(L" * nonce    : "); kull_m_string_wprintf_hex(des_buffer->data, des_buffer->dataLen, 0); sekcee_stdout(L"\n"); // try to leave it as is =)
									//sekcee_stdout(L" * SID      : "); kull_m_string_displaySID(pSid); sekcee_stdout(L"\n");
									//sekcee_stdout(L" * SHA1     : "); kull_m_string_wprintf_hex((PBYTE) des_buffer + cbOutput - SHA_DIGEST_LENGTH, SHA_DIGEST_LENGTH, 0); sekcee_stdout(L"\n");
									if(kull_m_crypto_hash(CALG_SHA1, des_buffer, cbOutput - SHA_DIGEST_LENGTH, digest, SHA_DIGEST_LENGTH))
									{
										//sekcee_stdout(L" > Calc SHA1: "); kull_m_string_wprintf_hex(digest, SHA_DIGEST_LENGTH, 0); sekcee_stdout(L"\n");
...
void kull_m_dpapi_displayPromptFlags(DWORD flags)
{
	...
	for(i = 0; i < ARRAYSIZE(DPAPIPromptFlagsToStrings); i++)
		if((flags >> i) & 1)
			sekcee_stdout(L"%s ; ", DPAPIPromptFlagsToStrings[i]);
}
...
void kull_m_dpapi_displayProtectionFlags(DWORD flags)
{
	...
	for(i = 0; i < ARRAYSIZE(DPAPIProtectFlagsToStrings); i++)
		if((flags >> i) & 1)
			sekcee_stdout(L"%s ; ", DPAPIProtectFlagsToStrings[i]);
	if(flags & CRYPTPROTECT_SYSTEM)
		sekcee_stdout(L"%s ; ", L"system");
}
...
void kull_m_dpapi_displayBlobFlags(DWORD flags)
{
	...
	for(i = 0; i < ARRAYSIZE(DPAPIBlobFlagsToStrings); i++)
		if((flags >> i) & 1)
			sekcee_stdout(L"%s ; ", DPAPIBlobFlagsToStrings[i]);
	if(flags & CRYPTPROTECT_SYSTEM)
		sekcee_stdout(L"%s ; ", L"system");
}