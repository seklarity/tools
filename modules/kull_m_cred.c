...
{
	sekcee_stdout(L"%*s" L"**CREDENTIAL**\n", level << 1, L"");
	if(cred)
	{
		sekcee_stdout(L"%*s" L"  credFlags      : %08x - %u\n", level << 1, L"", cred->credFlags, cred->credFlags);
		sekcee_stdout(L"%*s" L"  credSize       : %08x - %u\n", level << 1, L"", cred->credSize, cred->credSize);
		sekcee_stdout(L"%*s" L"  credUnk0       : %08x - %u\n\n", level << 1, L"", cred->credUnk0, cred->credUnk0);
		sekcee_stdout(L"%*s" L"  Type           : %08x - %u - %s\n", level << 1, L"", cred->Type, cred->Type, kull_m_cred_CredType(cred->Type));
		sekcee_stdout(L"%*s" L"  Flags          : %08x - %u\n", level << 1, L"", cred->Flags, cred->Flags);
		sekcee_stdout(L"%*s" L"  LastWritten    : ", level << 1, L""); kull_m_string_displayFileTime(&cred->LastWritten); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  unkFlagsOrSize : %08x - %u\n", level << 1, L"", cred->unkFlagsOrSize, cred->unkFlagsOrSize);
		sekcee_stdout(L"%*s" L"  Persist        : %08x - %u - %s\n", level << 1, L"", cred->Persist, cred->Persist, kull_m_cred_CredPersist(cred->Persist));
		sekcee_stdout(L"%*s" L"  AttributeCount : %08x - %u\n", level << 1, L"", cred->AttributeCount, cred->AttributeCount);
		sekcee_stdout(L"%*s" L"  unk0           : %08x - %u\n", level << 1, L"", cred->unk0, cred->unk0);
		sekcee_stdout(L"%*s" L"  unk1           : %08x - %u\n", level << 1, L"", cred->unk1, cred->unk1);
		sekcee_stdout(L"%*s" L"  TargetName     : %s\n", level << 1, L"", cred->TargetName);
		sekcee_stdout(L"%*s" L"  UnkData        : %s\n", level << 1, L"", cred->UnkData);
		sekcee_stdout(L"%*s" L"  Comment        : %s\n", level << 1, L"", cred->Comment);
		sekcee_stdout(L"%*s" L"  TargetAlias    : %s\n", level << 1, L"", cred->TargetAlias);
		sekcee_stdout(L"%*s" L"  UserName       : %s\n", level << 1, L"", cred->UserName);
		sekcee_stdout(L"%*s" L"  CredentialBlob : ", level << 1, L"");
		kull_m_string_printSuspectUnicodeString(cred->CredentialBlob, cred->CredentialBlobSize);
		sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  Attributes     : %u\n", level << 1, L"", cred->AttributeCount);
...
{
	sekcee_stdout(L"%*s" L"**ATTRIBUTE**\n", level << 1, L"");
	if(Attribute)
	{
		sekcee_stdout(L"%*s" L"  Flags   : %08x - %u\n", level << 1, L"", Attribute->Flags, Attribute->Flags);
		sekcee_stdout(L"%*s" L"  Keyword : %s\n", level << 1, L"", Attribute->Keyword);
		sekcee_stdout(L"%*s" L"  Value   : ", level << 1, L"");
...
{
	sekcee_stdout(L"%*s" L"**LEGACY CREDENTIAL**\n", level << 1, L"");
	if(cred)
	{
		sekcee_stdout(L"%*s" L"  credSize       : %08x - %u\n\n", level << 1, L"", cred->credSize, cred->credSize);
		sekcee_stdout(L"%*s" L"  Flags          : %08x - %u\n", level << 1, L"", cred->Flags, cred->Flags);
...
{
	sekcee_stdout(L"%*s" L"**VAULT POLICY**\n", level << 1, L"");
	if(policy)
	{
		sekcee_stdout(L"%*s" L"  version : %08x - %u\n", level << 1, L"", policy->version, policy->version);
		sekcee_stdout(L"%*s" L"  vault   : ", level << 1, L""); kull_m_string_displayGUID(&policy->vault); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  Name    : %s\n", level << 1, L"", policy->Name);
		sekcee_stdout(L"%*s" L"  unk0/1/2: %08x/%08x/%08x\n", level << 1, L"", policy->unk0, policy->unk1, policy->unk2);
		if(policy->key)
...
{
	sekcee_stdout(L"%*s" L"**VAULT POLICY KEY**\n", level << 1, L"");
	if(key)
	{
		sekcee_stdout(L"%*s" L"  unk0  : ", level << 1, L""); kull_m_string_displayGUID(&key->unk0); sekcee_stdout(L"\n");
		sekcee_stdout(L"%*s" L"  unk1  : ", level << 1, L""); kull_m_string_displayGUID(&key->unk1); sekcee_stdout(L"\n");
...
{
	DWORD i;
	sekcee_stdout(L"%*s" L"**VAULT CREDENTIAL CLEAR ATTRIBUTES**\n", level << 1, L"");
	if(clear)
	{
		sekcee_stdout(L"%*s" L"  version: %08x - %u\n", level << 1, L"", clear->version, clear->version);
		sekcee_stdout(L"%*s" L"  count  : %08x - %u\n", level << 1, L"", clear->count, clear->count);
		sekcee_stdout(L"%*s" L"  unk    : %08x - %u\n", level << 1, L"", clear->unk, clear->unk);
		if(clear->entries)
		{
			sekcee_stdout(L"\n");
			for(i = 0; i < clear->count; i++)
			{
				sekcee_stdout(L"%*s" L"  * ", level << 1, L"");
				switch(clear->entries[i]->id)
				{
				case 1:
					sekcee_stdout(L"ressource     : ");
					break;
				case 2:
					sekcee_stdout(L"identity      : ");
					break;
				case 3:
					sekcee_stdout(L"authenticator : ");
					break;
				default:
					sekcee_stdout(L"property %3u  : ", clear->entries[i]->id);
					break;
				}
