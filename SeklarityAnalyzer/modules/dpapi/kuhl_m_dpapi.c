				...
				if(kuhl_m_dpapi_unprotect_raw_or_blob(dataIn.pbData, dataIn.cbData, &description, argc, argv, NULL, 0, (LPVOID *) &dataOut.pbData, &dataOut.cbData, NULL))
				{
					...
					if(kull_m_string_args_byName(argc, argv, L"out", &outfile, NULL))
					{
						if(kull_m_file_writeData(outfile, dataOut.pbData, dataOut.cbData))
							sekcee_stdout(L"Write to file \'%s\' is OK\n", outfile);
					}
					else
					{
...
NTSTATUS kuhl_m_dpapi_protect(int argc, wchar_t * argv[]) // no support for protecting with RAW masterkey at this time
{
	...
	CRYPTPROTECT_PROMPTSTRUCT promptStructure = {sizeof(CRYPTPROTECT_PROMPTSTRUCT), CRYPTPROTECT_PROMPT_ON_PROTECT, NULL, SEKCEE}, *pPrompt;
	DWORD flags = 0, outputMode = 1;

	kull_m_string_args_byName(argc, argv, L"data", (PCWSTR *) &dataIn.pbData, SEKCEE);
	...
	sekcee_stdout(L"flags       : "); kull_m_dpapi_displayProtectionFlags(flags); sekcee_stdout(L"\n");
	sekcee_stdout(L"prompt flags: "); if(pPrompt) kull_m_dpapi_displayPromptFlags(pPrompt->dwPromptFlags); sekcee_stdout(L"\n");
	sekcee_stdout(L"entropy     : "); kull_m_string_wprintf_hex(dataEntropy.pbData, dataEntropy.cbData, 0); sekcee_stdout(L"\n\n");
	...
	if(kull_m_string_args_byName(argc, argv, L"in", &szIn, NULL))
	{
		if(kull_m_file_readData(szIn, &buffer, &szBuffer))
		{
			if(masterkeys = kull_m_dpapi_masterkeys_create(buffer))
			{
				...
				if(masterkeys->MasterKey && masterkeys->dwMasterKeyLen)
				{
					...
					if(masterkeys->dwFlags & 2)
					{
						if(pSystem && cbSystem)
						{
							...
							{
								sekcee_stdout(L"\n[masterkey] with DPAPI_SYSTEM (machine, then user): "); kull_m_string_wprintf_hex(pSystem + cbSystemOffset, 2 * SHA_DIGEST_LENGTH, 0); sekcee_stdout(L"\n");
							...
							{
								sekcee_stdout(L"\n[masterkey] with DPAPI_SYSTEM: "); kull_m_string_wprintf_hex(pSystem + cbSystemOffset, cbSystem - cbSystemOffset, 0); sekcee_stdout(L"\n");
					...
					else if(convertedSid)
					{
						...
						if(pHash)
						{
							...
							if(cbHash == LM_NTLM_HASH_LENGTH)
								sekcee_stdout(L" (ntlm type)\n");
							else if(cbHash == SHA_DIGEST_LENGTH)
								sekcee_stdout(L" (sha1 type)\n");
							else 
								sekcee_stdout(L" (?)\n");
				...
				//			sekcee_stdout(L" (but is not needed)");
				//	}
				//	sekcee_stdout(L"\n");
			...
			if(credhist = kull_m_dpapi_credhist_create(buffer, szBuffer))
			{
				...
				for(i = 0; prevSid && (i < credhist->__dwCount); i++)
				{
					if(ConvertSidToStringSid(prevSid, &convertedSid))
					{
						...
						{
							sekcee_stdout(L"\n  [entry %u] with SHA1 and SID: ", i); kull_m_string_wprintf_hex(passwordHash, sizeof(passwordHash), 0); sekcee_stdout(L"\n");
...
BOOL kuhl_m_dpapi_unprotect_raw_or_blob(LPCVOID pDataIn, DWORD dwDataInLen, LPWSTR *ppszDataDescr, int argc, wchar_t * argv[], LPCVOID pOptionalEntropy, DWORD dwOptionalEntropyLen, LPVOID *pDataOut, DWORD *dwDataOutLen, LPCWSTR pText)
{
	...
	CRYPTPROTECT_PROMPTSTRUCT promptStructure = {sizeof(CRYPTPROTECT_PROMPTSTRUCT), CRYPTPROTECT_PROMPT_ON_PROTECT | CRYPTPROTECT_PROMPT_ON_UNPROTECT | CRYPTPROTECT_PROMPT_STRONG, NULL, SEKCEE}, *pPrompt;
	...
	if(blob = kull_m_dpapi_blob_create(pDataIn))
	{
		...
		{
			if(pText)
				sekcee_stdout(L"%s", pText);

			if(isNormalAPI)
			{
				sekcee_stdout(L" * using CryptUnprotectData API\n");
			}
			
			if(entry)
			{
				sekcee_stdout(L" * volatile cache: ");
				kuhl_m_dpapi_oe_masterkey_descr(entry);
			}
			if(masterkey)
			{
				sekcee_stdout(L" * masterkey     : ");
				kull_m_string_wprintf_hex(masterkey, masterkeyLen, 0);
				sekcee_stdout(L"\n");
			}
			if(pPrompt)
			{
				sekcee_stdout(L" > prompt flags  : ");
...
void kuhl_m_dpapi_display_CredHist(PKULL_M_DPAPI_CREDHIST_ENTRY entry, LPCVOID ntlm, LPCVOID sha1)
{
	...
	sekcee_stdout(L"   "); kull_m_string_displaySID(entry->pSid); sekcee_stdout(L" -- "); kull_m_string_displayGUID(&entry->header.guid); sekcee_stdout(L"\n");
	sekcee_stdout(L"   > NTLM: "); kull_m_string_wprintf_hex(ntlm, LM_NTLM_HASH_LENGTH, 0); sekcee_stdout(L"\n");
