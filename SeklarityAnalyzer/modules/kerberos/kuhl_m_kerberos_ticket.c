...
{
	sekcee_stdout(L"\n\t   Start/End/MaxRenew: ");
	kull_m_string_displayLocalFileTime(&ticket->StartTime); sekcee_stdout(L" ; ");
	kull_m_string_displayLocalFileTime(&ticket->EndTime); sekcee_stdout(L" ; ");
	...
	if(ticket->Description.Buffer)
		sekcee_stdout(L" ( %wZ )", &ticket->Description);
...
void kuhl_m_kerberos_ticket_displayFlags(ULONG flags)
{
	...
	for(i = 0; i < ARRAYSIZE(TicketFlagsToStrings); i++)
		if((flags >> (i + 16)) & 1)
			sekcee_stdout(L"%s ; ", TicketFlagsToStrings[i]);
}
...
void kuhl_m_kerberos_ticket_displayExternalName(IN LPCWSTR prefix, IN PKERB_EXTERNAL_NAME pExternalName, IN PUNICODE_STRING pDomain)
{
	...
	if(pExternalName)
	{
		sekcee_stdout(L"(%02hu) : ", pExternalName->NameType);
		for(i = 0; i < pExternalName->NameCount; i++)
			sekcee_stdout(L"%wZ ; ", &pExternalName->Names[i]);
	}
	else sekcee_stdout(L"(--) : ");
	if(pDomain)
		sekcee_stdout(L"@ %wZ", pDomain);
}
