...
NTSTATUS kuhl_m_kerberos_purge(int argc, wchar_t * argv[])
{
	...
	if(NT_SUCCESS(status))
	{
		if(NT_SUCCESS(packageStatus))
			sekcee_stdout(L"Ticket(s) purge for current session is OK\n");
...
NTSTATUS kuhl_m_kerberos_tgt(int argc, wchar_t * argv[])
{
	...
	sekcee_stdout(L"Kerberos TGT of current session : ");
	if(NT_SUCCESS(status))
	{
		if(NT_SUCCESS(packageStatus))
		{
	...
	if(NT_SUCCESS(status))
	{
		if(NT_SUCCESS(packageStatus))
		{
			for(i = 0; i < pKerbCacheResponse->CountOfTickets; i++)
			{
				...
				sekcee_stdout(L"\n   Start/End/MaxRenew: ");
				kull_m_string_displayLocalFileTime((PFILETIME) &pKerbCacheResponse->Tickets[i].StartTime); sekcee_stdout(L" ; ");
				kull_m_string_displayLocalFileTime((PFILETIME) &pKerbCacheResponse->Tickets[i].EndTime); sekcee_stdout(L" ; ");
				...
				if(export)
				{
					...
					if(pKerbRetrieveRequest = (PKERB_RETRIEVE_TKT_REQUEST) LocalAlloc(LPTR, szData)) // LPTR implicates KERB_ETYPE_NULL
					{
						...
						if(NT_SUCCESS(status))
						{
							if(NT_SUCCESS(packageStatus))
							{
								if(filename = kuhl_m_kerberos_generateFileName(i, &pKerbCacheResponse->Tickets[i], SEKCEE_KERBEROS_EXT))
								{
		...
		if(pKerbRetrieveRequest = (PKERB_RETRIEVE_TKT_REQUEST) LocalAlloc(LPTR, szData))
		{
			...
			if(NT_SUCCESS(status))
			{
				if(NT_SUCCESS(packageStatus))
				{
					...
					sekcee_stdout(L"   * Ticket Encryption Type & kvno not representative at screen\n");
					if(isNoCache && isExport)
					sekcee_stdout(L"   * NoCache: exported ticket may vary with informations at screen\n");
					...
					if(isExport)
						filename = kuhl_m_kerberos_generateFileName_short(&ticket, SEKCEE_KERBEROS_EXT);
								...
								if(NT_SUCCESS(status))
								{
									if(kull_m_string_stringToHex(szKey, key, pCSystem->KeySize))
									{
										...
										*(PULONGLONG) &lifeTimeData.TicketRenew += (ULONGLONG) 10000000 * 60 * wcstoul(szLifetime, NULL, 0);
										...
										sekcee_stdout(L"Groups Id : *");
										for(i = 0; i < nbGroups; i++)
											sekcee_stdout(L"%u ", groups[i].RelativeId);
										if(nbSids)
										{
											sekcee_stdout(L"\nExtra SIDs: ");
											for(i = 0; i < nbSids; i++)
										...
										sekcee_stdout(L"\nServiceKey: ");
										kull_m_string_wprintf_hex(key, pCSystem->KeySize, 0); sekcee_stdout(L" - %s\n", kuhl_m_kerberos_ticket_etype(keyType));
										if(szService)
											sekcee_stdout(L"Service   : %s\n", szService);
										if(szTarget)
											sekcee_stdout(L"Target    : %s\n", szTarget);
										sekcee_stdout(L"Lifetime  : ");
										kull_m_string_displayLocalFileTime(&lifeTimeData.TicketStart); sekcee_stdout(L" ; ");
										kull_m_string_displayLocalFileTime(&lifeTimeData.TicketEnd); sekcee_stdout(L" ; ");
										kull_m_string_displayLocalFileTime(&lifeTimeData.TicketRenew); sekcee_stdout(L"\n");

										sekcee_stdout(L"-> Ticket : %s\n\n", isPtt ? L"** Pass The Ticket **" : filename);
	...
	if(kuhl_m_pac_validationInfo_to_PAC(&validationInfo, SignatureType, pClaimsSet, &pacType, &pacTypeSize))
	{
		...
		if(NT_SUCCESS(status))
		{
			...
			if(BerApp_EncTicketPart = kuhl_m_kerberos_ticket_createAppEncTicketPart(&ticket, pacType, pacTypeSize))
			{
				...
				if(NT_SUCCESS(status))
				{
					sekcee_stdout(L" * EncTicketPart encrypted\n");
					if(BerApp_KrbCred = kuhl_m_kerberos_ticket_createAppKrbCred(&ticket, FALSE))
						sekcee_stdout(L" * KrbCred generated\n");
				}
...
NTSTATUS kuhl_m_kerberos_hash_data(LONG keyType, PCUNICODE_STRING pString, PCUNICODE_STRING pSalt, DWORD count)
{
	...
	if(NT_SUCCESS(status))
	{
		...
		{
			status = (SEKCEE_NT_MAJOR_VERSION < 6) ? pCSystem->HashPassword_NT5(pString, buffer) : pCSystem->HashPassword_NT6(pString, pSalt, count, buffer);
			if(NT_SUCCESS(status))
			{
				sekcee_stdout(L"\t* %s ", kuhl_m_kerberos_ticket_etype(keyType));
...
NTSTATUS kuhl_m_kerberos_decode(int argc, wchar_t * argv[])
{
	...
	if(szKey)
	{
		sekcee_stdout(L"Key is OK (%08x - %u)\n", keyType, keyLen);
				...
				if(kull_m_string_stringToHex(szKey, key, keyLen))												
				{
					...
					if(NT_SUCCESS(status))
					{
						if(kull_m_file_writeData(szOut, decData, decSize))
							sekcee_stdout(L"DEC data saved to file! (%s)\n", szOut);
						else PRINT_ERROR_AUTO(L"\nkull_m_file_writeData");
						LocalFree(decData);
					}
...
//		{
//			if(NT_SUCCESS(packageStatus))
//				sekcee_stdout(L"KerbChangePasswordMessage is OK\n");
...
//		{
//			if(NT_SUCCESS(packageStatus))
//				sekcee_stdout(L"kerbSetPasswordRequest is OK\n");
