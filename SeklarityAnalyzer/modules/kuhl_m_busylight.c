...
NTSTATUS kuhl_m_busylight_status(int argc, wchar_t * argv[])
{
	...
	if(isBusyLight)
	{
		sekcee_stdout(L"BusyLight detected\n");
		for(cur = kuhl_m_busylight_devices; cur; cur = cur->next)
		{
			sekcee_stdout(L"\n[%3u] %s\n"
			...
			for(i = 0; i < ARRAYSIZE(kuhl_m_busylight_capabilities_to_String); i++)
			{
				if((cur->deviceId->Capabilities >> i) & 1)
					sekcee_stdout(L"%s, ", kuhl_m_busylight_capabilities_to_String[i]);
			}
			sekcee_stdout(L")\n");

			sekcee_stdout(L"  Device Handle: 0x%p\n", cur->hBusy);
			if(cur->hBusy)
			{
				if(kull_m_busylight_device_read_infos(cur, &info))
				{
					sekcee_stdout(L"    Status     : 0x%02x\n", info.status);
					sekcee_stdout(L"    ProductId  : %S\n", info.ProductId);
					sekcee_stdout(L"    CostumerId : %S\n", info.CostumerId);
					sekcee_stdout(L"    Model      : %S\n", info.Model);
					sekcee_stdout(L"    Serial     : %S\n", info.Serial);
					sekcee_stdout(L"    Mfg_ID     : %S\n", info.Mfg_ID);
					sekcee_stdout(L"    Mfg_Date   : %S\n", info.Mfg_Date);
					sekcee_stdout(L"    swrelease  : %S\n", info.swrelease);
				}
			}
			sekcee_stdout(L"  KeepAlive Thread: 0x%p (%u ms)\n"
...
NTSTATUS kuhl_m_busylight_list(int argc, wchar_t * argv[])
{
	...
	if(isBusyLight)
	{
		for(cur = kuhl_m_busylight_devices; cur; cur = cur->next)
		{
			sekcee_stdout(L"[%3u] %s ( ", cur->id, cur->deviceId->Description);
			for(i = 0; i < ARRAYSIZE(kuhl_m_busylight_capabilities_to_String); i++)
			{
				if((cur->deviceId->Capabilities >> i) & 1)
					sekcee_stdout(L"%s, ", kuhl_m_busylight_capabilities_to_String[i]);
