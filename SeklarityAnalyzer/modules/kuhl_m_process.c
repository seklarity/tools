...
{
	DWORD i;
	sekcee_stdout(L"%u\t%wZ", pSystemProcessInformation->UniqueProcessId, &pSystemProcessInformation->ImageName);
	if(*(PBOOL) pvArg && pSystemProcessInformation->NumberOfThreads)
	{
		sekcee_stdout(L" (");
		for(i = 0; i < pSystemProcessInformation->NumberOfThreads; i++)
			sekcee_stdout(L"%u ", pSystemProcessInformation->Threads[i].ClientId.UniqueThread);
		sekcee_stdout(L")");
	}
	sekcee_stdout(L"\n");
	return TRUE;
...
BOOL CALLBACK kuhl_m_process_exports_callback_module_exportedEntry(PKULL_M_PROCESS_EXPORTED_ENTRY pExportedEntryInformations, PVOID pvArg)
{
	...
	if(pExportedEntryInformations->name)
		sekcee_stdout(L"\t%u", pExportedEntryInformations->hint);
	else
		sekcee_stdout(L"\t ");

	if((pExportedEntryInformations->function.address))
		sekcee_stdout(L"\t%p", pExportedEntryInformations->function.address);
	else
		sekcee_stdout(L"\t ");

	if(pExportedEntryInformations->name)
		sekcee_stdout(L"\t%S", pExportedEntryInformations->name);
	else
		sekcee_stdout(L"\t ");

	if(pExportedEntryInformations->redirect)
		sekcee_stdout(L"\t-> %S", pExportedEntryInformations->redirect);
	return TRUE;
}
...
BOOL CALLBACK kuhl_m_process_imports_callback_module_importedEntry(PKULL_M_PROCESS_IMPORTED_ENTRY pImportedEntryInformations, PVOID pvArg)
{
	...
	if(pImportedEntryInformations->name)
		sekcee_stdout(L"%S", pImportedEntryInformations->name);
	else
		sekcee_stdout(L"#%u", pImportedEntryInformations->ordinal);
	return TRUE;
}