				...
				else 
				{
					for(i = 0; pBerVal[i]; i++)
					{
						...
						if((_wcsicmp(pAttribute, L"sIDHistory") == 0))
						{
							...
							{
								sekcee_stdout(L" ( %s -- %s\\%s )", kull_m_token_getSidNameUse(nameUse), domain, name);
								LocalFree(name);
								LocalFree(domain);
							}
						}
